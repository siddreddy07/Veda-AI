...
**GitHub:** https://github.com/siddreddy07/Veda-AI
...
The application first uses Google Cloud Vision to extract text and positional bounding boxes from the uploaded documents. Groq then structures the questions, groups the student's answer content, semantically maps answers to questions, and grades the mapped responses. Highlight regions are calculated from the original OCR bounding boxes rather than generated by the AI.
...
- PDF OCR uses the synchronous Google Vision `batchAnnotateFiles` API, which processes at most the first 5 pages of a PDF (the async API supports up to 2000 pages but requires Google Cloud Storage, which is not used here).
